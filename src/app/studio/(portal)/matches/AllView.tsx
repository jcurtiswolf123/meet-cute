...
// The read-only ledger: every introduction ever made, grouped by where it
// landed. Closed matches live only here, which is why the view survives the
// merge rather than folding into the board.
...
export async function totalMatchCount(): Promise<number> {
            ...
            <section key={group.key}>
              ...
              <ul className="mt-4 divide-y divide-line/70 overflow-hidden rounded-lg border border-line bg-panel">
                ...
                {rows.map((m) => (
                  <li
                    key={m.id}
                    className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 px-4 py-3.5 transition-colors hover:bg-studio-canvas/70"
                  >
